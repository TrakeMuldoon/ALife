...
namespace ALife.Core.NewGeometry
{
    /// <summary>
    ...
    public struct Angle
    {
        ...
        {
            Range<double> range = new Range<double>(GeometryConstants.ZeroPi, GeometryConstants.TwoPi);
            double actualRadians = range.CircularClampValue(radians);
            double degrees = RadiansToDegrees(actualRadians);
            return new Angle(degrees);
        }
        ...
        {
            return HashCodeHelper.Combine(_degrees, _radians);
        }
        ...
        {
            int piesInRads = (int)(Radians / GeometryConstants.Pi);
            double remaining = Radians % GeometryConstants.Pi;
            return $"Deg: {Degrees:0.00}, Rads: {piesInRads}pi + {remaining} ({Radians})";
        }
