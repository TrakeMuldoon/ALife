...
namespace ALife.Core.Utility.Random
{
    ...
    public class FastRandom : IRandom
    {
        ...
        /// <summary>
        /// Initialises a new instance using an int value as seed. This constructor signature is provided to maintain
        ...
        public int Next()
        {
            ...
            // Handle the special case where the value int.MaxValue is generated. This is outside of the range of
            // permitted values, so we therefore call Next() to try again.
        ...
        public int Next(int upperBound)
        {
            ...
            // The explicit int cast before the first multiplication gives better performance. See comments in NextDouble.
        ...
        public int Next(int lowerBound, int upperBound)
        {
            ...
            // 31 bits of precision will suffice if range<=int.MaxValue. This allows us to cast to an int and gain a
        ...
        public void NextBytes(byte[] buffer)
        {
            ...
            {
                // Generate 4 bytes. Increased performance is achieved by generating 4 random bytes per loop. Also note
                // that no mask needs to be applied to zero out the higher order bytes before casting because the cast
                // ignores thos bytes. Thanks to Stefan Troschütz for pointing this out.
                t = (x ^ (x << 11));
                x = y; y = z; z = w;
                w = (w ^ (w >> 19)) ^ (t ^ (t >> 8));
        ...
        public double NextDouble()
        {
            ...
            // Here we can gain a 2x speed improvement by generating a value that can be cast to an int instead of the
            // more easily available uint. If we then explicitly cast to an int the compiler will then cast the int to a
            // double to perform the multiplication, this final cast is a lot faster than casting from a uint to a
            // double. The extra cast to an int is very fast (the allocated bits remain the same) and so the overall
            // effect of the extra cast is a significant performance improvement.
            //
            // Also note that the loss of one bit of precision is equivalent to what occurs within System.Random.
            return (REAL_UNIT_INT * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8)))));
        }

        // /// <summary> /// A version of NextBytes that uses a pointer to set 4 bytes of the byte buffer in one
        // operation /// thus providing a nice speedup. The loop is also partially unrolled to allow
        // out-of-order-execution, /// this results in about a x2 speedup on an AMD Athlon. Thus performance may vary
        // wildly on different CPUs /// depending on the number of execution units available. /// /// Another
        // significant speedup is obtained by setting the 4 bytes by indexing pDWord (e.g. pDWord[i++]=w) /// instead of
        // adjusting it dereferencing it (e.g. *pDWord++=w). /// /// Note that this routine requires the unsafe
        // compilation flag to be specified and so is commented out by default. /// </summary> /// <param
        // name="buffer"></param> public unsafe void NextBytesUnsafe(byte[] buffer) { if(buffer.Length % 8 != 0) throw
        // new ArgumentException("Buffer length must be divisible by 8", "buffer");
        //
        // uint x=this.x, y=this.y, z=this.z, w=this.w;
        //
        // fixed(byte* pByte0 = buffer) { uint* pDWord = (uint*)pByte0; for(int i=0, len=buffer.Length>>2; i < len;
        // i+=2) { uint t=(x^(x<<11)); x=y; y=z; z=w; pDWord[i] = w = (w^(w>>19))^(t^(t>>8));
        //
        // t=(x^(x<<11)); x=y; y=z; z=w; pDWord[i+1] = w = (w^(w>>19))^(t^(t>>8)); } }
        //
        // this.x=x; this.y=y; this.z=z; this.w=w; }
        ...
        // Buffer 32 bits in bitBuffer, return 1 at a time, keep track of how many have been returned with bitBufferIdx.
        uint bitBuffer;
